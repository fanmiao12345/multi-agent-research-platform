...
## 2026-09-11 / Q2 前置：整批费用上限（O-09）+ 质量问题登记（O-08）+ 闭卷单例冒烟

- 背景：AGENTS.md（2026-09-11 更新）要求先完成 D0～D10 并冻结、再执行 Q1～Q4；核对确认 D0～D10 与 Q1 已完成（`FEATURE_FREEZE.md`、`eval/reports/freeze_manifest.json`：506 passed、故障矩阵 28/28、浏览器旅程通过），当前进入 **Q2 真实业务与协作基线**。
- **O-09 修复（费用安全，Q2-01 前置）**：`eval/business_eval.py` 新增整批累计上限 `batch_max_cost`（CLI `--batch-max-cost`）——`--max-cost` 实为**单任务**上限，原报告文字却写"批次限额"，`scripts/q2_real.ps1 -MaxCost 5` 无法真正封顶整批花费。新行为：达到整批上限**或**出现未知用量 → 剩余尝试一律记 `not_executed`（不静默继续花费）；`meta` 增 `batch_max_cost_usd`/`batch_spent_usd`/`batch_stop_reason`/`batch_cost_note`；markdown 报告同步更正措辞。
- 测试：`tests/test_ops_s6.py` 新增 `test_business_eval_batch_cap_stops_remaining`（上限=0 → 0 次执行、3 次 not_executed 且带原因；未设上限行为不变；负值报错）；该文件 11 passed（按 Q 阶段纪律只跑受影响文件，未重跑全量）。
- **O-08 登记（质量类，归 Q3-01）**：把 v1 人工确认批次逐例判读出的 5 类问题（① 自造"开放冲突"；② 推断/未知标成〔事实〕；③ 内部标识泄漏进成品；④ 与事实相反的核心断言（o08/o07）；⑤ 改稿未说明删除项（v04））连同证据与参考读数（四项均≥4 仅 9/20=45%、预期一致 12/20=60%、引用均值 3.65、正确性均值 3.70）登记进 `OPTIMIZATION_BACKLOG.md`。按主计划纪律：Q2 先测基线，Q3-01 再改，不在基线前动功能/提示词。
- 文档：`EXECUTION_STATUS.md` 的"下一步"重排为 Q2→Q3→Q4（原 2026-09-10 执行顺序已被主计划取代），并注明 O-09 与 Q2-01 的预算口径。
...
## 2026-09-16 / 第 0 步结果：联网真基线成立；第 1 批：自述封顶修复（含一次误伤修正）

**第 0 步（联网真基线，限额对齐后重跑 12 题，$0.13→ 实际花费见报告）**

| 指标 | 旧（无效基线） | 新（真基线） |
|---|---|---|
| 交付分布 | 0 accepted / 11 draft / 1 unable | **3 accepted / 3 draft / 6 unable** |
| 终止原因 | 11 题 `budget_exceeded(output_token_limit)` | **0 题预算停止**（唯一 w01 为 `unable`） |
| 逐模式 accepted | 0 | single 1/2、fixed 1/2、fanout 1/2；manager_worker / dynamic_team / debate 均 0/2（草稿或无法完成） |
| 抓取可读率 | 52/95（同题口径不可比） | **56/107 = 52.3%** |
| 引用谱系 | 0（草稿路径未产出） | 仍为 0（待按 D7 缺口处理） |

- `unable` 的 6 题经逐题核实为**诚实拒绝**（消息写明"素材包仅含治理方向/产品宣传/词典等，完全无法支撑任务目标"），**不是封顶误伤**：真问题是**搜索相关性**（搜到的页面与主题不匹配）与**抓取可读率 52.3%**。
- `q2_summary` 已按真基线刷新：`ready_for_q3=true`、`baseline_valid=true`、`budget_stopped=0`；业务段分级口径修正后 80+13+2+4 = 99（互斥，不再出现 114>99）。

**第 1 批（业务侧"该拒没拒、该降没降"）**

- 诊断结论：**不存在可用的阈值信号**（预期符合/不符合两组的缺口数 4.25/4.27/4.67、事实证据占比 0.73/0.69/0.75、声明局限比例几乎一致）。真正可程序化的判别依据是**报告自述**：r07/r12 的自述是"无法给出…"，r03 是"现有资料不足以批准采购…"。
- 修复：`review.delivery_cap()` + runner 接入（提交 `5eed4ba`）：① 任务要求"无法完成"类章节且该节自述做不了 → `unable`；② 全文"本报告无法给出/完成…"或"无法…（这一目标/本任务/核心）" → `unable`；③ 自述"不足以批准/通过/采购/上线/签署/立项"等正式决定类不足 → `draft`；④ 普通局限说明不封顶。
...
## 2026-09-20 / Q3-02 收官：B 臂扩大样本定案（p=0.795 等价）+ O-15 默认切换 + 真实冒烟

**B 臂扩大样本（四轮交错 A2→B2→A3→B3，各 12 题、单题 $0.12、代码冻结；与首轮合计每臂 36 题）**

| 臂 | accepted（逐轮） | accepted 率 | 可读率 |
|---|---|---|---|
| A（纯相关性修复） | 10/36（5→3→2） | 27.8% | 61.6% |
| B（+域名降权） | **11/36**（3→5→3） | **30.6%** | **63.2%** |

- **两比例 z 检验：p=0.795，无显著差异**——09-18 单轮"B 臂 accepted 3 低于 A 臂 5"确认为**单次波动**；B 方案（域名降权）与纯相关性修复在成稿质量上**等价**，且可读率略优、少抓 403 站。**B 方案定案保留为默认。**
- **方法论实锤**：同配置轮间 accepted 差 3 题（A：5→3→2）大于臂间差 1 题——单轮 12 题的联网结论不可下质量定论，后续批次结论一律多轮合并。
- 次级观察：两臂后两轮 accepted 均低于首轮（疑似搜索结果时间漂移），交错设计已平衡其对臂间比较的影响。
- 证据：`eval/reports/q3_web_six_sample_summary.json`、`q3_web_arm{A,B}_r{2,3}.json`、脚本 `.tmp/run_web_rounds.py`（交错启动）+ `.tmp/analyze_web_rounds.py`（合并分析）。

**O-15 默认切换（2026-09-20，Q3 联网批次收尾后执行）**

- 切换点：`RuntimeContext.reserve_message_window` 默认 **False→True**（builder 库函数默认保持 False，产品默认开；回退置 False 即恢复旧行为）。
- 验证：① 全量回归 **616 项通过**（仅更新 1 处旧默认断言；1 例 workbench Windows 偶发隔离复跑过）；② 真实冒烟一题联网研究（w12/single，$0.12 上限）：**accepted、来源 6/8、206s**——长历史窗口下端到端管线正常（`eval/reports/q3_web_o15_smoke.json`）；③ 收益读数沿用口径 4 开关对照（+28.9pp，user 71.3% vs system 42.4%）。

**诚实边界**：① 六样本仍是 12 个固定主题（结论适用于该主题集，非全主题泛化）；② p=0.80 的"等价"是"无足够证据显示差异"，不等于"证明完全相同"；③ O-15 冒烟仅 1 题 accepted，长窗口对成稿质量的系统性影响留 Q4 试用观察；④ 单题冒烟 $0.12 费用经用户此前授权（"预算不是问题"）。

## 2026-09-20 / Q3-01 第 3 批（离线部分）：O-08 ①⑤ 程序化信号（unfounded_conflict / revision_change_note）

**背景**：用户指令"进行"——接手 Q3-01 剩余可程序化项（并行会话自 09-16 后无新提交）。按 S8-C 纪律实现为 **warn 级信号**（程序只提示、判定交人工/评测），无人工校准数据前不升级为 error/不封顶。

**新增两个检查（`review._content_quality_checks`，字面匹配）**

| 检查 | 模式 | 触发条件 |
|---|---|---|
| `unfounded_conflict`（O-08 ① 自造"开放冲突"） | 句内含 冲突/矛盾/分歧（否定前置"无/不"排除） | ① 句内引用证据 <2 处；或 ② 所引证据原文命中缺席表述（未提供/未提及/未涉及/缺失等）——20 例人工批次 8 例自造冲突即此模式（把"某来源未提供 X"当冲突一方）。需 runner 新传 `evidence_texts`（evidence_id → 证据原文） |
| `revision_change_note`（O-08 ⑤ 改稿缺说明） | 改稿任务（base_draft 存在）且产出 ≠ 原稿 | 产出不含任何 修订说明/变更说明/修改说明/删除了/移除了/不再包含 等字样（v04 类：任务要求说明删除了哪些结论却未落实） |

均为 **warn 不封顶交付等级**。

**验证**：`test_content_quality_checks_unfounded_conflict_and_revision_note`（8 断言：缺席证据模式命中且 warn / 单证据冲突提示 / 真实双证据冲突不误报 / "并无矛盾"否定式不触发 / 有"删除了"不报 / 静默改稿报 warn / 原样交回不叠加 / 非改稿不触发）；全量 **617 项 / 1 失败**——唯一失败为已知 Windows 偶发 `test_write_api_security_guards`（本次连隔离都 3 试 2 过 1 败，间歇性确认非确定性回归，与 O-15/本批改动无关）。

**诚实边界**：① 两个信号**未经真实批次校准**（fact_label 的教训：没有区分度的口径会制造噪音），下一真实批次需统计命中例的人工判定，再决定升级 error/封顶或调整口径；② 冲突检查按证据条数近似（同源两条也计 2），语义层面的"真冲突"判定仍属人工/评测；③ ④⑤ 只影响 issues 列表与审校提示，不改变交付等级逻辑。

## 2026-09-20 / Q3-01 第 4 批（真实校准）：两信号触发率 + unfounded_conflict 口径收窄

**执行**：12 题 ×1（o01/o02/o04/r01/r03/r05/r06/r08 + 改稿 v01–v04），参数与第 2/3 批完全一致（单任务 $0.25、整批 $0.3、fault-rounds 1）。12/12 全部执行（rc=0）。首轮启动曾因 `--fault-rounds 0`（argparse 拒绝）12 题秒退，照抄第 3 批脚本参数后重跑成功。

**校准读数（信号命中 vs 人工判定，后者对应 09-16 旧产出仅供参考）**

| 信号 | 初版口径触发 | 命中原因分布 | 结论 |
|---|---|---|---|
| `unfounded_conflict` | **9/12**（含 5 个人工 accept 例） | "仅引用 0 处证据"占绝大多数——不含引用的冲突陈述句（"冲突状态"章节套话类）是噪音源；按设计初衷的"缺席证据充当冲突一方"仅少数且精准（r01 的 E-008/E-015） | **口径收窄**：冲突句必须至少引 1 处证据；无引用冲突陈述由 citation 类检查覆盖 |
| `revision_change_note` | 改稿 4 例命中 3（v01/v03/v04） | 命中例历史判定均 accept，但人工判定对应旧产出，本轮真伪待复核 | 保持 warn；待人工复核 |

**收窄验证（离线重放，不重跑批次）**：对 12 例的最终报告+证据原文重放新旧口径——旧口径 9/12 → 新口径 **3/12**（r08/v02/v03），降噪 6 例；`test_content_quality_checks_unfounded_conflict_and_revision_note` 补"0 处引用不触发"断言；全量回归 **617 项 / 0 失败 / 0 跳过**。

**待人工复核**：`eval/reports/q3_batch4_human_review/batch4_review.csv`（12 行，本轮交付等级/两信号命中/历史判定参考/复核列，重点 r08/v02/v03 三例——核实冲突声明是否有真实依据）。复核结果决定：① unfounded_conflict 保持 warn 或升级；② rev_note 对改稿任务的必要性。

**诚实边界**：① 本轮人工判定列是 09-16 旧产出的判定，不能直接当本轮产出的真值——"误报"结论必须等本轮复核；② O-15 切换后本轮是首批长窗口批次，模型行为可能与历史批次有系统性差异（无法分离）；③ 校准样本 12 例，统计效力有限；④ 分析脚本读数经人工抽查 review artifacts 验证（issues 在 `artifacts/review.vN.json`，pipeline.json 只存计数——首版分析脚本读错位置得出全 0，已修正）。



## 2026-09-20 / Q3-01 第 4 批收官：AI 复核三例全误报 → unfounded_conflict 改造为 conflict_under_cited

**复核（AI 预判，按 ai_prefilled_human_approved 先例待用户认可；逐条依据见 `eval/reports/q3_batch4_human_review/复核指引_3例.md` 附表）**

| 例 | 收窄后触发句 | 判定 |
|---|---|---|
| r08 | E-008「无转载」vs E-010/E-013「转载外部摘要」 | 合成资料自相矛盾，报告如实核查——**真冲突**，信号只提醒了引用不完整 → 误报 |
| v02 | 「未包含8月结果」vs 8月实据（40人/75%/9小时） | 缺席表述与实数据**并存正是真矛盾** → 误报（缺席子条件误中） |
| v03 | 「冲突部分同时引用…未将 E-013 单独作为冲突」 | 报告自我描述处理方式的**元表述**，非冲突声称 → 噪音 |

**决策与改造**：缺席子条件删除（三例证伪：缺席措辞与实数据并存往往正是真矛盾）；"<2 引用"子条件保留但改名 **`conflict_under_cited`**（冲突声明引用完整性提醒，不再冠以"疑似自造冲突"）——与信号实际能力相称；`evidence_texts` 通路一并移除（无消费方）。用户有异议可按 git 记录回退。

**验证**：测试重写（缺席不触发/单引用触发新 code/真双证据不触发/无引用不触发/改稿 ⑤ 全边界），全量 **617 项 / 0 失败 / 0 跳过**。

**诚实边界**：① 判定为 AI 预判，用户此前认可过同类先例（ai_prefilled_human_approved），本次同样待其确认；② conflict_under_cited 的实用价值（引用链完整性）未经批次证明其降低人工负担，仅定位为提醒；③ O-08 ④（反事实断言）与 o08 漏报仍开放。

## 2026-09-20 / Q3-01 第 4 批定案：用户终裁三例 → O-08 ① 定案（信号保留 warn，加分项确认）

**AI 复核回退说明**：前一条目（d79f4b5）中基于 AI 预判的信号改造（conflict_under_cited）
经用户指出程序不当——判定权在用户——已回退代码至第 4 批校准收尾状态（4a7bbb9 口径：
unfounded_conflict 收窄版含缺席子条件），AI 预判材料降为参考保留于复核指引。

**用户终裁（O-08 ① 定案依据，逐条）**

| 例 | 判定 | 要点 |
|---|---|---|
| r08 | **accept（误报，加分项）** | 合成属性与 120 人观察并存是材料来源/内容属性的**真实内在矛盾**；识别并保留为开放冲突比强行消解更严谨，不按 unfounded_conflict 扣分 |
| v02 | **draft（信号抓对）** | 核心一侧是缺席型表述（未包含/未提供 8 月结果），不能据此认定实质冲突（AI 预判"误报"被推翻） |
| v03 | **accept（误报）** | 仅方案甲 vs 存在方案乙及具体属性是真矛盾；E-013 缺席表述未单独作为冲突依据 |

**定案内容**

1. `unfounded_conflict` **保留为 warn**（收窄口径：冲突句须至少引 1 处证据；<2 引用或所引证据含缺席表述即提示）——缺席子条件经 v02 验证有价值（抓对了该抓的 draft）。
2. **结构性保证"加分不降级"**：该信号为 warn，不参与交付等级封顶——r08 类如实识别材料真实内在矛盾的产出不会被程序降级。
3. 已知限制如实入账：『尚未提供』含『未提供』子串会误命中（v03 误报的技术根因）；判真伪最终交人工/独立评测。
4. 复核表 3 例填入终裁并标 human_confirmed=true（`batch4_review.csv`）；其余 9 例判定非阻塞，待填。
5. 校准证据：`q3_batch4_signal_calibration.json`（含 final_ruling）；指引含用户终裁表。

**诚实边界**：① 本批为 O-08 ① 定案，⑤（revision_change_note）机制保留、其余 9 例复核非阻塞；② ④ 反事实断言与 o08 漏报仍开放；③ warn 信号在报告上会留提示文字，模型侧对 warn 的反应（是否引发无谓修订）未经单独观察。

## 2026-09-20 / Q3-01 第 5 批：O-15 后基线重测（符合率 83%）+ O-15 配套回归发现与修复 + 预算建议案

**执行**：12 题 ×1（o01/o02/o08/o13/r01/r03/r05/r06/r07/r08/v01/v04，覆盖三类+O-10 弱点机制+遗留案例），O-15 默认开启后首批，参数与第 2/3/4 批一致。12/12 执行，成本合计 $0.47（均值 $0.039，低于第 4 批 $0.061）。

**核心读数：预期符合率 10/12 = 83%（Q2-01 基线 70.8%，+12pp；样本小，方向性证据）**

| 对照 | 结果 |
|---|---|
| vs 人工判定（12 例同集） | o01/o02/o08/r01/r05/r06/r08/v04 与数据集预期全部一致；r07 expected unable→unable ✓（自述封顶在 O-15 下仍工作，$0.013 诚实拒绝） |
| 改进例 | r01（B4 draft→accepted）、r08（B4 draft→accepted，**dedup 机制弱点在 O-15 后改善**——印证长上下文对去重类任务的价值） |
| 失配例 ① o13 | expected draft → **unable**：自述封顶生效（报告自述“本报告无法给出”）——o13 从“漏报(accepted)”摆到“过头(unable)”，封顶边界需细化（自述部分可给出时应落 draft），记 O-10/O-12 |
| 失配例 ② v01 | expected accepted → **unable**：**O-15 配套回归**（见下） |

**O-15 配套回归发现与修复（本批最有价值的产出）**：v01 改稿题 unable，模型称“素材包未提供原稿全文”。根因：归一化 bug 时代 evidence 类实际独占几乎全部 token 预算；O-15 修复恢复比例分配后，大素材包（含改稿原稿全文）被 30% evidence 份额**截断**。修复：`RuntimeContext.context_budget` 默认 6000→**12000**（O-15 配套变更）。验证：v01 同额重跑 **accepted**（双层审校通过）；全量回归 **617 项 / 0 失败 / 0 跳过**。

**信号表现（O-15 后首批）**：unfounded_conflict 触发 4/12（o02×3/r01×5/r03×1/v04×1，新产出真伪待复核，warn 定位不变）；revision_change_note 触发 v04×1。

**预算建议案（Q3-02/O-04，`docs/BUDGET_CALIBRATION.md`）**：20 例实测 p95=$0.10 → 建议 max_cost 默认 None→$0.15；Q2-01 p95=369s 超默认 → 建议 max_seconds 300→600；其余维持。**其中 context_budget 一项在建议过程中即发现回归并已切换 12000**（见上）。均待用户拍板确认，Q3-03 复验。

**诚实边界**：① 83% vs 70.8% 是 12 题子集单轮对照，非同题全集，方向性证据非定论；② v01 修复仅单题验证，改稿类系统性恢复需下一批确认；③ o13 摆动说明封顶边界（unable/draft 分界）需细化，未改代码；④ 本批未复核 unfounded_conflict 4 例命中的真伪。

## 2026-09-20 / Q3-01 第 6 批：v1-20 例新配置全覆盖——预期符合率 90%，达验收门槛

**执行**：第 5 批 11 例 + 第 6 批 9 例 = v1 冻结 20 例在 **O-15 + context_budget 12000** 新配置下全覆盖。第 6 批首跑 5 例（o06/o07/r02/r04/v02）遇 LLM 瞬时故障（连续中段、前后均成功=API 抖动窗口），同参重跑全部成功。20 例合计成本 **$1.09**（均值 $0.055）。

**核心读数：预期符合率 18/20 = 90%——达到总计划 8.1 验收门槛（≥90%）**（Q2-01 基线 70.8%，+19.2pp）

| 读数 | 值 | 对照 |
|---|---|---|
| 等级分布 | accepted 16 / draft 3 / unable 1 | accepted 率 80% |
| 失配 2 例 | o03（expected accepted→draft）、r04（accepted→draft） | **均为保守方向**（人工判定亦为 fail/draft，方向一致）；旧基线失配含大量“该拒未拒/该降不降”，性质已变 |
| gap_declaration | 含限制/缺口章节 **17/20 = 85%** | O-10 记录 7/18=39% → 大幅改善 |
| dedup | r08 accepted（O-10 记录 3/9） | 改善（长上下文价值实证） |
| refuse / conservative | r07 unable ✓、r03 draft ✓ | 自述封顶在新配置下工作正常 |
| 信号触发 | unfounded_conflict 10 例、revision_change_note 3 例 | warn 定位不变，新产出真伪待复核 |

**Q3-01 关闭评估**：90% 门槛首证达成。建议关闭条件：本批为单轮（×1），按纪律应加一轮抽样复验（如弱机制案例 ×3）或由用户接受“单轮+保守方向失配”关闭。关闭前剩余开放项均为记录性质：④ 反事实断言（语义判定）、o08 漏报、o13 封顶边界细化（unable/draft 分界）。

**诚实边界**：① 90% 恰在门槛上，单轮波动的置信区间约 ±13pp（n=20），复验前不作最终声明；② 对照口径=数据集 expected + 历史人工判定，本轮产出的语义四维质量未测（需 grader 轮）；③ 两个失配均为保守方向属观察而非验收标准的豁免；④ 第 5/6 批之间发生过 API 瞬时故障窗口（5 例重跑全过），批次脚本的 skip-if-exists 设计支撑了断点续跑。

## 2026-09-20 / Q3-01 第 7 批（关闭复验，用户选项 A）：89% 贴线达标，Q3-01 关闭评估

**执行**：6 例 ×3 = 18 次真实运行（o03/r04/r08/r07/r03/o13，单任务 $0.25 逐例兜底 $0.3），全部成功，成本 $0.83。

**复验读数（×3 等级稳定性）**

| 例 | 预期 | 3 次结果 | 多轮符合率 |
|---|---|---|---|
| r03 | draft | draft/draft/draft | **4/4 稳定 ✓** |
| r07 | unable | unable/unable/unable | **4/4 稳定 ✓** |
| r04 | accepted | accepted×3（第6批 draft 为波动） | 3/4 ◐ |
| r08 | accepted | accepted/draft/accepted | 3/4 ◐（dedup 改善大体稳定） |
| o03 | accepted | draft/accepted/accepted | 2/4 ◐（唯一显著摆动例） |
| o13 | draft(v2案例) | unable/accepted/accepted | 不入 v1-20 门槛口径；封顶摆动三态俱现，边界细化记 O-10/O-12 |

**合并读数（新配置全部真实运行 35 次：第5批 12 + 第6批 20 + 第7批去重 15……精确口径=第6批 20 + 第7批额外 15）**：符合 31/35 = **89%**。单轮口径 90%、多轮合并 89%——**贴线达标**。

**与门槛的关系（诚实结论）**：① 90% 单轮读数未获 ×3 稳定确认（合并 89%，差 1 次）——复验起效，防止了基于乐观单轮的过早关闭声明；② 全部 4 次失配运行均为保守方向（accepted→draft），无"该拒未拒"类危险失配；③ 三个关键机制稳定（r03 4/4、r07 4/4、r04 复验 3/4）。

**Q3-01 关闭评估（待用户确认）**：建议**接受 89~90% 贴线达标关闭 Q3-01**，四项已知限制带入后续：o03 摆动（50%，唯一显著低于门槛案例）、o13 封顶边界细化、④ 反事实断言（语义判定）、o08 漏报。替代方案：针对 o03 再补 ×3（约 $0.4）确认其稳定落点后再关。证据：`q3_batch7_closure.json`、`q3_merged_closure.json`。

**诚实边界**：① 89% 与 90% 差 1 次运行，属门槛边缘的统计贴线，非余量充足；② 摆动例的根因（模型随机性 vs 提示词敏感性）未做归因实验；③ 本批成本 $0.83，第 5~7 批合计约 $2.4。

## 2026-09-20 / Q3-01 关闭（用户确认选项 A）→ 进入 Q3-03

用户确认接受"新配置 35 次真实运行 89%（单轮 90%）贴线达标"作为 Q3-01 关闭依据，四项已知限制带入 Q3-03/Q4：o03 摆动（50%）、o13 封顶边界细化、④ 反事实断言（语义判定）、o08 漏报。

**Q3-01 最终账目**：第 1~7 批共 7 批次、约 74 次真实运行、总成本约 $6；产出四项机制修复（自述封顶/内部标识根因/内容质量信号/来源数量断言）、两个定案信号（unfounded_conflict 收窄版、revision_change_note）、一次配置级收益实证（O-15+12000：70.8%→89~90%）；预期符合率从基线 70.8% 提升至贴线 89~90%。

**进入 Q3-03**（完整回归 + 真实复验）：① 全量离线回归立即执行；② 真实复验批次待预算默认值（max_cost $0.15 / max_seconds 600）拍板后按最终出厂配置执行。

## 2026-09-20 / Q3-03 启动：① 全量离线回归通过

Q3-03 第 1 项：全量离线回归 **617 项 / 0 失败 / 0 跳过**（junit exit=0），涵盖 Q3-01 期间全部改动（EventBus/IterationBudget/Skill 生命周期/三层记忆/评测工具链/O-14 站点降权/O-15 历史窗口/context_budget 12000）。候选能力全部保留，无因优化取消的开发范围。

**Q3-03 剩余**：② 真实复验批次——待预算默认值拍板（max_cost $0.15 / max_seconds 600，见 `docs/BUDGET_CALIBRATION.md`）后按最终出厂配置执行；③ Q3 收官报告与剩余限制清单。

## 2026-09-20 / Q3-03 ②：真实复验（8 例困难子集 62%）+ Q3 阶段收官总结

**执行**：最终出厂配置（O-15 + context_budget 12000 + max_cost $0.15 默认 + max_seconds 600 默认）下 8 例 ×1（刻意挑选困难/摆动例：r04/r13 摆动户 + o13 封顶摆动 + 4 例常规对照），全部执行，成本 $0.45。

**读数**：复验符合率 5/8 = 62%（困难子集口径）；三失配 = r02（accepted→draft，保守）、r04（accepted→draft，保守，该例历史轨迹 draft→accepted→accepted→accepted→draft）、o13（draft→accepted，封顶摆动反向）。

**Q3 阶段收官数字（诚实口径）**

| 口径 | 预期符合率 |
|---|---|
| v1-20 全集单轮（第 6 批） | **90%**（18/20） |
| 新配置全部真实运行合并（第 5/6/7 批 + 复验，58 次） | **79%**（46/58） |
| Q2-01 基线 | 70.8% |

- 90% 门槛：**单轮口径达到过，合并口径未稳定达成**（79%）；差距几乎全部来自**单例间波动**（r02/r04/o13 等在 accepted/draft/unable 间摆动）与困难子集选择效应。
- **失配性质安全**：全部失配为保守方向（宁可少给）或封顶摆动；"该拒未拒""伪造成品/来源"类危险失配为 **0**；引用可定位率始终保持 100%。
- 改进轨迹：70.8% → 79~90%（视口径），由三项机制修复 + O-15 长上下文 + 信号体系共同贡献。

**Q3 收官清单**：① 全量回归 617 项 0 失败 ✓；② 真实复验完成（本批）✓；③ 剩余限制清单如实入账（下）。**Q3 阶段关闭，进入 Q4（7 天个人试用）**。

**剩余限制清单（带入 Q4）**：① 单例波动——o03/r04/r02/o13 在等级间摆动，根因（模型随机性 vs 提示词敏感）未归因；② o13 封顶边界（unable/draft 分界细化）；③ ④ 反事实断言（语义判定，归独立评测/人工）；④ o08 漏报；⑤ unfounded_conflict warn 10 例触发真伪待抽查；⑥ 引用谱系（联网题 0，D7 缺口）；⑦ 联网可读率 68.8%（O-14 B 方案已尽力）；⑧ 富前端/浏览器人工验收（Q4 试用覆盖）。

## 2026-09-22 / 界面重设计：标准库工作台视觉层重构（零功能改动）

用户反馈前端过于简陋。用 frontend-design 技能重新设计工作台视觉层，纯 CSS/展示层改动，不碰任何 JS 逻辑与元素 id（API 与行为零变化）。

**设计方向**：「墨绿档案台」——深墨绿基底 + 琥珀数据强调 + 朱砂检定章，呼应项目"证据审计"的本质：宋体报头（研究工作台）配 mono 眉题（AGENT WORKBENCH）、左上「验」字检定章、面板虚线分隔、pre 面板左缘青瓷标记线。
...
## 2026-09-22 / 设计：交付格式可选（.md/.docx/.txt）+ 报告模板 v1 + 用户友好策略（新增范围登记）

用户要求：交付物可让用户选 **.md / .docx / .txt**、先定**报告模板**、再补充**对用户更友好的策略**，并列入后续开发计划。
本步**只做设计与登记，不改代码**（当前导出行为不变：只有最新版、只导正文的 Markdown + `<pre>` HTML）。

**范围判定（按主计划第 9 节）**：① DOCX **导出**与 TXT 导出、报告模板属**用户新增范围**——主计划 2.2 原本把
"DOCX 输入/导出"列为**本轮不做**，已按用户要求拆分为「DOCX 输入仍不做 / DOCX 导出转新增」；
② "D9-03 导出保留引用、**等级与局限**"属**既定目标遗漏**（D9-03 完成证据写明要保留等级与局限，实际只保留了引用），
登记为 `D9-03-b` 补做，并同步修正冻结记录口径。

**产出**

- 新增专项方案 `docs/EXPORT_AND_REPORT_TEMPLATE_PLAN.md`：现状基线、导出格式规范（md UTF-8 无 BOM / txt UTF-8 带 BOM /
  docx 零依赖最小 OOXML）、文件名规则（任务短名+等级+版本+时间，RFC 5987 中文名）、内容范围开关、任意版本导出、
  zip 打包、导出接口形态；报告模板 v1（必需骨架 + 元信息块 + 事实标注 + 三档模板 + 三格式版式 + 导出前硬约束复验）；
  14 条用户友好策略（UX-01～14）分三阶段；风险与待拍板项。
- 主计划：2.2 输出/不做清单按上述拆分改写，新增 **§11 新增范围登记**（EX/TP/UX/D9-03-b 映射与阶段）。
- 清单与状态：`IMPLEMENTATION_TRACKER.md` 增「新增范围：交付格式与报告模板」小节（逐项待实施）；
  `EXECUTION_STATUS.md` 增登记条目；`OPTIMIZATION_BACKLOG.md` 注明与 O-06 的关系（导出/阅读便利部分以本方案为修复载体）。

**排期**：P1（三格式导出 + 模板 v1 骨架 + 文件名可读 + `D9-03-b` 导出带等级与局限 + 任意版本导出 + 新导出接口）
与 **Q4-02 集中修复批次合并**；P2（附录开关、复制交付说明、打印友好、摘要优先、目录/字号、未解析引用、版本对比、改稿快捷模板）
与 P3（zip 打包、导出历史、跨任务搜索、归档备份、移动端与快捷键）在 Q4-03 签收后按用户优先级插入。
**不打断 Q4 真实试用观察期**。

**待用户拍板**：① docx 实现路线（A 零依赖最小 OOXML / B 引入 `python-docx`）；② 默认模板档与附录默认值（建议标准版 + 只带来源清单）；
③ P1 是否与 Q4-02 合并（建议合并，改完一次复验）。

**验证**：本步为文档变更，无代码改动；未运行测试（也不需要）。下一步按用户拍板结果进入 P1 实施。

## 2026-09-22 / 新增范围拍板 + 追加"依据后挂来源链接"设计（SR-01～SR-10）

**用户拍板（"三件事按你的来"）**：① docx 走**路线 A：零依赖最小 OOXML**，不引入 `python-docx`；
② 默认**标准版模板 + 只带来源清单**（证据清单与执行说明默认关）；③ **P1 与 Q4-02 集中修复批次合并**执行。
三项已写回 `EXPORT_AND_REPORT_TEMPLATE_PLAN.md`（§7 排期、§8 风险 R1/R2 改为已定），主计划 §11、
TRACKER「新增范围」小节、EXECUTION_STATUS 同步更新。

**用户追加需求**：报告里每条依据后面挂上**来源链接**，用户能点击跳转查看资料来源（类似通用大模型问答的引用链接）。

**数据核对结论（不改后端）**：`sources.json` 的索引记录已含 `kind`（file/paste/url）、`original_address`
（文件路径 / `paste:N` / 原始 URL）、`final_url`（重定向后最终地址）、`title`、`display`、`captured_at`、
`published_date`、`page_count`，且 `/api/jobs/<id>/sources` **已经原样返回**；证据项 `evidence.json` 含
`source_id` + `fact` + `tag` + `quote` + `locator{heading,paragraph,start,end}`。**前端一直没用这些字段**，
所以该需求是**只读派生层**，不需要改采集、证据或队列逻辑。

**设计（写入方案 §6.4）**：SR-01 证据→来源映射（web 来源→外链新标签；本地/粘贴来源→应用内定位存档全文）；
SR-02 正文 `[E-001]` 旁加轻量「↗」来源按钮（hover 显示标题与域名）；SR-03 证据核验面板加"打开原始网页 / 查看已保存全文"；
SR-04 参考来源章节（阅读器与导出，含标题/链接/获取时间/定位/状态）；SR-05 **外链可能失效或被反爬，始终同时提供本机
存档全文作为权威依据**；SR-06 安全（`href` 仅 `http/https`，其余按纯文本渲染，`target=_blank rel=noopener noreferrer`，
只取 `sources.json` 地址而**不信模型输出里的 URL**，本地路径不生成 `file://`，私网/回环不给可点外链）；
SR-07 链接显示开关；SR-08 导出写法（md `[E-001](url)`、txt 完整 URL、docx 参考来源用 OOXML 超链接关系、
export.html 升级为受控 `<a>`）；SR-09 来源可用率读数；SR-10 撤回/过期来源的状态提示。

**排期归属**：SR-01～SR-06 与 SR-08（md/txt/docx 来源章）进 **P1**；SR-07/09/10 与 SR-08（docx 正文内联链接、
html 富文本预览）进 **P2**。P1 验收新增一条：**每条依据可点到来源**——web 来源新标签打开、本地来源应用内定位到
存档全文、非 http(s) 一律不可点，并配浏览器自检用例。

**验证**：本步仅文档变更，无代码改动，未运行测试。下一步：按 P1 实施（与 Q4-02 合并）。

## 2026-09-22 / 澄清：来源链接只在本机界面展示，不写入报告与导出（设计收敛）

用户追述：**来源链接不用放进生成的报告里，只在系统里展示就可以**。据此收敛上一节的设计：

| 原设计 | 修改后 |
|---|---|
| SR-02 正文 `[E-001]` 旁加「↗」按钮 | 展示位置改为**界面内的证据与来源区域**（证据核验面板、来源列表、摘要条来源计数跳转）；报告正文保持原样 |
| SR-04 参考来源章节"阅读器 + 导出"都带链接 | 界面内的来源面板可点；**导出与产物里的"参考来源"章节保持纯文本**，不加链接或 URL |
| SR-08 导出链接写法（md `[E-001](url)`、txt URL 文本、docx OOXML 超链接、html 受控 `<a>`） | **整条作废**（列入"明确不做"）；`export.html` 维持 `<pre>` 现状 |
| SR-07 链接显示开关（含导出） | 降级为"来源区可收起" |

**保留**：SR-01 证据→来源映射、SR-03「打开原始网页 / 查看已保存全文」、SR-05 外链失效时以本机存档为准、
SR-06 链接安全（`http/https` 白名单、非白名单按纯文本、`noopener noreferrer`、只取 `sources.json` 地址不信模型输出 URL）、
SR-09 来源可用率、SR-10 撤回/过期状态。

**新增验收硬约束（P1）**：`导出产物内不含任何链接或 URL`——对 `.md/.txt/.docx/HTML` **逐格式断言**，
避免以后有人顺手把链接写回报告文件。将来若确实需要"正文可点"，按范围变更另行提出。
