...
- 初始基线148项；B1后174项，B2后199项离线测试通过（当前全量见 EXECUTION_STATUS）。业务案例已定义 **33 项**（2026-09-10 v2：v1 冻结 20 + v2 机制扩充 13，每例带 mechanisms 标签供分机制统计比例），故障案例 10 项；v1 的 20 例已真实执行 1 轮（2026-09-09/10，闭卷前口径），人工确认评分未完成。详见`RESEARCH_WRITING_ACCEPTANCE.md`与`EXECUTION_STATUS.md`。
...
- ~~不以增加 Debate、Dynamic Team 等编排名称作为交付目标。~~
  **（2026-09-10 方向变更）** 改为：不以"编排名称/模式数量"作为交付目标，而以"**系统自己选型是否正确 + 与固定流程的质量/成本对比证据**"作为交付目标；系统自适应多智能体进入产品主线，设计见 `DYNAMIC_ORCHESTRATION_PLAN.md`。
...
**S2-03/04 升级为主题研究的前置条件（2026-09-10 审阅后明确）**：「用户给链接」与「系统自己找资料」是两项能力；"只给研究主题"的产品目标以接通一个真实搜索服务为前提。三档行为：有资料→只用给定资料；无资料、允许联网且搜索可用→自动搜索并读取正文；无资料且搜索不可用→明确说明缺少资料，不生成貌似经过研究的报告。服务商选型、搜索预算与验收标准（输入主题→找到原文→引用可核对→标出缺口）见 `DYNAMIC_ORCHESTRATION_PLAN.md` 第 2.E 节；首版候选百度千帆搜索（约 ¥0.036/次、日免费 100 次，接入前按官网核验），博查 / Brave / Tavily 按实测缺口作为第二来源。注意：Bing Web Search API 已于 2025-08-11 退役，不再列为方案。
...
- Debate/Dynamic Team 保留实验入口；新策略优于业务基线后再进入产品路径。
  **（2026-09-10 修订）** 这些模式可由调度智能体自动选中，但**默认模式仍取"够用的最省模式"**；任何模式被设为默认前，必须给出同批业务任务上的质量与成本对比（见 `DYNAMIC_ORCHESTRATION_PLAN.md` 第 8 节）。
