...
| O-04 | 默认预算中点、成稿预留、并发与模型路由参数 | 用户已定中点规则，参数需要同等任务规模下的实际成本校准 | Q3-02 | **建议案已产出（2026-09-20，`docs/BUDGET_CALIBRATION.md`），待拍板**：实测成本样本 20 例（第3/4批）+ Q2-01 99 例：均值 $0.048~0.061、p95 $0.10、draft 均价高 accepted 35%；建议 ① `max_cost` 默认 None→**$0.15**（1.5×p95 兜底，当前真实模式无默认费用上限属裸奔）；② `max_seconds` 300→**600**（Q2-01 p95=369s 超默认 23%，口径盲区）；③ max_calls/max_output_tokens/context_budget 维持（170+ 次真实任务 0 次预算停止）。落定时机：随 Q3-03 复验 ||
