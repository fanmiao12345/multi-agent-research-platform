...
| O-11 | **联网研究质量（Q2-02）**：~~12 个真实主题、六模式各 2 个，交付 0 accepted（11 草稿 + 1 无法完成）；抓取可读率 52/95 = 54.7%；引用谱系 0；$0.13~~ **上述读数已判定为"无效基线"**（2026-09-16 复核）：11/12 题是被 **CLI 默认输出上限 8192 token** 掐断（`budget_exceeded/output_token_limit`，47–59 秒），不是研究能力读数——`eval/web_baseline.py` 未传 `--max-calls/--max-output-tokens/--max-seconds`（业务批为 60/200000/1500）。**唯一跑完的 w01** 终止原因是 `unable`：搜到的全是节假日/日历类无关页面，素材包明确列出政策文本等缺口后诚实拒绝——**这才是 O-11 的真问题：搜索相关性与正文可读率**。旧报告归档为 `eval/reports/q2_web_baseline_invalid_8192.json`（带 `invalid_baseline/invalid_reason`）。 | `eval/reports/q2_web_baseline_invalid_8192.json`（逐题 rc/终止原因/来源数）；`.tmp/q2_fix_verify.txt`（限额对照核验） | Q3-01（等修复后的真基线） | 修复已提交（限额对齐），**真基线重跑中**；相关性/可读率待真基线出来后定位 |
