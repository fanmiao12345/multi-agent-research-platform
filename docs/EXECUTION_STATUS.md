...
最近更新：2026-09-11。Q1-01～03 已通过：全量离线回归 506、故障矩阵 28/28、真实浏览器与安装恢复完成。Q2 起需要真实批次预算和人工评分。
...
- Q2～Q4 其余：工具链就绪；**Q2-02 联网基线（≥10 真实主题 + 模式覆盖）未运行**——为 ready_for_q3=false 的当前缺口，需用户确认预算后执行；Q3 调参与 Q4 试用未开始。
- 下一步：用户确认预算后执行 **Q2-02**（q2_web.ps1，联网六模式/主题基线）→ 重跑 q2_summary 使 ready_for_q3=true → Q2-03 汇总定稿（含分机制比例）→ Q3。
...
- **Q2-02**：12 个公开主题（六种协作方式各 2 个）真实联网执行 —— **12/12 执行、模式覆盖全达标**，但**交付 0 accepted**（11 草稿 + 1 无法完成）、抓取来源可读率 **52/95 = 54.7%**、引用谱系链接 **0**、费用 **$0.13**。结论：能搜到、能读正文，但"只给主题的联网研究"目前基本只能交付草稿 → 列为 **Q3 第一优先**（优化清单 **O-11**）。
- **Q2-03 汇总**：`eval/reports/q2_summary.json/.md`，**`ready_for_q3=true`**（人工确认 94 行 + 六模式覆盖达标）。
- 本轮工具修复（`eval/web_baseline.py`）：① 子进程中文输出 GBK→UTF-8 容错解码（首次运行因此崩溃、前两题作废）；② 每题后增量写盘 + 逐题进度打印 + 单题异常不拖垮整批；③ 覆盖口径修正为"实际执行过即计入"（原口径只算 exit 0，导致六模式覆盖恒为 0），`q2_web_baseline.json` 已按新口径重算。
