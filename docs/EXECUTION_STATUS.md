...
- **新增范围登记（2026-09-22，用户明确要求）**：交付物导出可选 **.md / .docx / .txt**、先定报告模板、补充用户友好策略，并追加**依据后挂来源链接**（点击跳转查看资料来源，类似通用大模型问答的引用链接）。性质是**新增范围**（主计划 2.2 原将 DOCX 输入/导出列为本轮不做，现拆分为：DOCX 输入仍不做、**DOCX 导出转新增**），其中"D9-03 导出保留引用、**等级与局限**"属**既定目标遗漏**，登记为 `D9-03-b` 补做。设计与分阶段验收见 [EXPORT_AND_REPORT_TEMPLATE_PLAN.md](EXPORT_AND_REPORT_TEMPLATE_PLAN.md)，主计划 §11、TRACKER「新增范围」小节已登记。
- **新增范围决策已拍板（2026-09-22，用户确认"按你的来"）**：① docx 走**零依赖最小 OOXML**（不引入 `python-docx`）；② 默认**标准版模板 + 只带来源清单**（证据清单/执行说明默认关）；③ **P1 与 Q4-02 集中修复批次合并**。来源链接数据侧已具备：`sources.json` 索引含 `original_address`/`final_url`/`title`/`captured_at`，`/api/jobs/<id>/sources` 已返回，前端此前未使用；证据项含 `source_id`+`locator`，因此 SR 属**只读派生层**、不改采集与证据逻辑。排期：P1（三格式导出 + 模板 v1 骨架 + 文件名可读 + 导出带等级/局限 + SR-01～06/08 来源章链接）、P2/P3 在 Q4-03 签收后按用户优先级插入，**不打断 Q4 真实试用观察期**。当前状态：**未开始（仅设计已定稿）**。
