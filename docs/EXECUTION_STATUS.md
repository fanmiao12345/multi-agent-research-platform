...
- Q2～Q4：用户自测工具链已就绪，包括真实批次、联网六模式、人工评分、汇总、对比、7 天试用和签收；尚未由用户运行。
- 下一步：用户执行 **Q2-01**；前置条件为预算、人工评分和后续 7 天试用时间。
...
- 人工评分工作台 `eval/reports/q2_human_workbench/combined_scores.csv`（99 行）待填写/确认；**未确认前 `ready_for_q3=false`**，上述读数不得当作业务验收通过。
