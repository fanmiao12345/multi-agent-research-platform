...
- **S8 主线**：S8-01 契约与调度智能体 → S8-02 执行接线 → S8-03 护栏 → S8-04 入口展示 → S8-05 真实对比（fixed vs auto vs 多智能体，需预算）→ S8-06 定默认模式。
- **先做纠偏**（免费）：评测口径重算、关闭开卷、禁止断言语义判定——否则后续任何对比数字都不可信。
- **待用户确认的默认值（草案，见设计文档）**：方案是否需人工点头（默认展示计划后再跑）、递归深度 2、派生总数 ≤12、并发 ≤3、子预算 ≤父预算 40%、简单题默认 single/fixed。
