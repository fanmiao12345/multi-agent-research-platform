...
- 界面优化（2026-09-22，用户视角信息架构）：按总计划 1.3/3.1/D9 与 Q4 指南重排为 4 个用户视图（新建任务 / 任务与成果 / 运行观测 / 配置与统计），暴露既有的"允许联网研究"和"先看计划再开始"，交付信息按"方式/依据/花费/等级"展示且过程阶段默认折叠；统计改为按本地任务表聚合。**修 4 个用户可见缺陷**：文本类端点被 JSON 二次转义（报告阅读器此前显示带 `\n` 的转义文本）、**待输入链路此前实际未接通**（status 仍为 queued → 常驻 worker 会抢先执行；已加 waiting_input 挂起 + requeue + 取消收敛 + `input_request.json` 执行前白名单）、无 hash 首屏视图路由抛错、任务列表缺任务描述。
- 界面优化验证（2026-09-22）：新增回归 `test_waiting_input_held_while_worker_alive_then_requeued`（去掉修复单跑即失败，`assert 'completed' == 'waiting_input'`）；全量 **611 项 / 0 失败 / 0 跳过**（exit=0；中途一次全量 1 例 `test_workbench_s5::test_write_api_security_guards` 连接错误，隔离复跑 7 项通过——O-07 同型）。合成数据 Playwright 逐视图自检 0 控制台错误，证据目录见下条（上一版目录已随 V3 合并清理）。**真实模型/真实资料的人工浏览器验收仍属 Q4 用户试用，不能用离线截图替代**。
- **前端改用 Research Console V3（2026-09-22，用户提供的第三版）**：`INDEX_HTML` 整体替换为 hash 路由的应用结构——`#/home` 首页（目标输入 + 资料/联网/先看计划/高级设置 + 最近任务 + 常见任务）、`#/tasks` 任务中心（搜索 + 全部/进行中/已交付/需要处理）、`#/job/<id>` 任务详情（结果 / 资料与产物 / 执行过程 / 版本与改稿，顶部固定执行方式·交付等级·来源·用量费用），`#/observe` 运行观测、`#/settings` 配置与评测；**后端接口与执行语义未改**。合并时补齐 8 处差异：任务时间 1970 错乱（Unix 秒按毫秒解析）、"先看计划"退化成 JSON（应读 `plan.subtasks`/`plan.reason`）、补充输入与恢复后轮询不重启（`openJob` 早退）、刚入队任务立刻 404（目录就绪前不请求来源/产物）、待补充"作为补充资料"会丢原资料（改为追加）、配置页缺数据边界说明、「阶段」列暴露内部英文词、**`/api/config` 搜索状态在 `settings=None` 时永远显示未配置**（改为按模型诊断同口径回落 `.env`）。设计源回写使应用脚本幂等。验证：py_compile 通过、全量 **611 项 0 失败**、合成数据 Playwright 走完整路径（含真实提交一次并跳转任务详情）截图 13 张、0 控制台错误、0 个 HTTP ≥400，证据 `eval/reports/workbench_ui_v3/`。清理：删除 `multi-agent-research-console-v2/`、`workbench.py.before-research-console-v3.bak`、上一版证据目录与 `.tmp` 临时产物。
- **新增范围登记（2026-09-22，用户明确要求）**：交付物导出可选 **.md / .docx / .txt**、先定报告模板、补充用户友好策略，并追加**依据对应的来源链接**（点击跳转查看资料来源）。性质是**新增范围**（主计划 2.2 原将 DOCX 输入/导出列为本轮不做，现拆分为：DOCX 输入仍不做、**DOCX 导出转新增**），其中"D9-03 导出保留引用、**等级与局限**"属**既定目标遗漏**，登记为 `D9-03-b` 补做。设计与分阶段验收见 [EXPORT_AND_REPORT_TEMPLATE_PLAN.md](EXPORT_AND_REPORT_TEMPLATE_PLAN.md)，主计划 §11、TRACKER「新增范围」小节已登记。
- **新增范围决策已拍板（2026-09-22，用户确认"按你的来"）**：① docx 走**零依赖最小 OOXML**（不引入 `python-docx`）；② 默认**标准版模板 + 只带来源清单**（证据清单/执行说明默认关）；③ **P1 与 Q4-02 集中修复批次合并**。来源链接数据侧已具备：`sources.json` 索引含 `original_address`/`final_url`/`title`/`captured_at`，`/api/jobs/<id>/sources` 已返回，前端此前未使用；证据项含 `source_id`+`locator`，因此 SR 属**只读派生层**、不改采集与证据逻辑。
- **来源链接展示范围已澄清（2026-09-22，用户追述）**：来源链接**只在本机系统界面展示与跳转**（证据核验面板、来源列表），**不写进报告产物，也不写进任何导出文件**（.md/.txt/.docx/HTML 均不含链接或 URL）——因此该需求是**纯前端展示层**工作，不触碰成稿与导出；原设计里的"导出链接写法"作废，改为 P1 的"导出内不含链接"逐格式断言验收项。排期：P1（三格式导出 + 模板 v1 骨架 + 文件名可读 + 导出带等级/局限 + SR-01～06 界面内来源入口）、P2/P3 在 Q4-03 签收后按用户优先级插入，**不打断 Q4 真实试用观察期**。当前状态：**未开始（仅设计已定稿）**。
