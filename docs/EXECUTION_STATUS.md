...
- **待用户确认预算**：repeats=3 的 60 次真实批次（估算 ≈$5，链+评测者）→ 得到稳定业务达标率；这是 S6-04 的验收门槛。
- **人工权威评分**：用已生成的 20 份评分表填写后 `python -m eval.human_scores ingest`（human_confirmed=true），才会得到可对外声明的业务通过率。
- **语义短板（correctness 3.85 持平）**：候选做法——链内审校换更强模型（如 v4-pro）缩小"链内 accepted 与独立评测"的语义差；或按证据缺口分级修订。
- **剥离提示效应**：增加"仅结构提示（blind）"批次开关，把必需章节与关键事实分开度量。
- 其余待办：S2 搜索服务商接入、S2-11 PDF、S5 剩余体验项、S7（有界并行/检索优化/模型路由/可选格式）、10 个故障案例两轮、7 天个人试用、单 Agent/手工基线对比、全新 venv 安装验证。
