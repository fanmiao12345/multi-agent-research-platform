...
- **新增范围登记（2026-09-22，用户明确要求）**：交付物导出可选 **.md / .docx / .txt**、先定报告模板、并补充用户友好策略。性质是**新增范围**（主计划 2.2 原将 DOCX 输入/导出列为本轮不做，现拆分为：DOCX 输入仍不做、**DOCX 导出转新增**），其中"D9-03 导出保留引用、**等级与局限**"属**既定目标遗漏**，登记为 `D9-03-b` 补做。设计与分阶段验收见 [EXPORT_AND_REPORT_TEMPLATE_PLAN.md](EXPORT_AND_REPORT_TEMPLATE_PLAN.md)，主计划 §11、TRACKER「新增范围」小节已登记；排期：**P1 与 Q4-02 集中修复批次合并**（三格式导出 + 模板 v1 骨架 + 文件名可读 + 导出带等级/局限），P2/P3 在 Q4-03 签收后按用户优先级插入，**不打断 Q4 真实试用观察期**。待用户拍板：docx 实现路线（零依赖最小实现 / 引入 `python-docx`）、默认模板档与附录默认值、P1 是否与 Q4-02 合并。当前状态：**未开始（仅设计已定稿）**。
